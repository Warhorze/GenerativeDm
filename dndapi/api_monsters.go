...
func (a *MonstersAPIService) ApiMonstersGetExecute(r ApiApiMonstersGetRequest) (*APIReferenceList, *http.Response, error) {
	...
	if r.challengeRating != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "challenge_rating", s.Index(i), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "challenge_rating", t, "multi")
		}
	}
