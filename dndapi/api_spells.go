...
func (a *SpellsAPIService) ApiSpellsGetExecute(r ApiApiSpellsGetRequest) (*APIReferenceList, *http.Response, error) {
	...
	if r.level != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "level", s.Index(i), "multi")
			}
		} else {
	...
	if r.school != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "school", s.Index(i), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "school", t, "multi")
		}
	}
